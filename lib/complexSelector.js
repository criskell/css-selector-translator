...
const translateComplexSelector = (tokens) => {
  ...
  const compoundSelectors = split.map((combinatorOrTokenList, index) => {
    ...
    return `. Este elemento deve ser encontrado como ${combinatorTranslation} de...\n`;
    })
    .join("") + ".";

  const startMessage = split.length > 1 ? `Começando do lado direito, temos:\n` : "";

  return startMessage + compoundSelectors;
};
